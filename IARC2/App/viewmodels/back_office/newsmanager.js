define(['services/utilities', 'services/httpService', 'services/displayService'], function (utilities, httpService, displayService) {
    ...
    var getData = function () {
        httpService.get("/ws/bo/news/get.php?d=" + Date.now()).done(function (data) { itemList(data); }).error(utilities.handleError);
    }

    this.remove = function (item) {
        if (confirm('You are about to delete this user. Are you sure?')) {
            httpService.post("/ws/news.php?d=" + Date.now(), { 'user': item }).done(function (data) {
