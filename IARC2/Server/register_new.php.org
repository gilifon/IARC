...
if (!empty($filename)){
$upload_dir = __DIR__.'/../members/images/';
...
';
$mpdf = new mPDF('', 'A4', '12', 'Times', 15, 15, 40, 16, 9, 9);
$mpdf->SetImportUse();
$mpdf->AddPage();
$pagecount = $mpdf->SetSourceFile('template.pdf');
$tplId = $mpdf->ImportPage($pagecount);
$mpdf->SetWatermarkImage($upload_dir.$filename, 1, array(35,45), array(15,55));
...
$mpdf->SetAutoFont();
$mpdf->WriteHTML($html);
$mpdf->Output("/home1/iarcorg/public_html/members/applications/".$ID_pdf,"F");
/***************************************************************************/

/*************************** Generate new CSV file *******************/
$upload_dir_csv = __DIR__.'/../members/csv/';
...
$file_pdf = __DIR__.'/../members/applications/'.$timestamp.'.pdf';
$file_csv = __DIR__.'/../members/csv/'.$timestamp.'.csv';
$file_img = __DIR__.'/../members/images/'.$filename;
$file_rcp = __DIR__.'/../members/payments/'.$paymentfilename;

$file_img_ext = substr($file_img, -3);
$file_rcp_ext = substr($file_rcp, -3);

$to = "4x6hp@iarc.org;4z5sm@iarc.org;4z5sl@iarc.org;$email";
if ($isDebug) $to = "4z5sm@iarc.org;4z5sl@iarc.org;gill@shamir.co.il;gilifon@gmail.com;yuli.kaplan@gmail.com;yulik@ifn-solutions.com";
$from = "info@iarc.org"; 

$files = array();
$files[0]= $file_csv;
$files[1]= $file_pdf;
//$files[2]= $file_img.".".$file_img_ext;
//$files[3]= $file_rcp.".".$file_rcp_ext;
$files[2]= $file_img;
$files[3]= $file_rcp;

multi_attach_mail($to, $files, "info@iarc.org");
