<!DOCTYPE html>
<html dir="rtl">
<head>
<meta charset="UTF-8">
<title>איזור מוגן</title>
</head>
    <body>
    <font face="Arial">
    <p>איזור מוגן</p>
<?
ini_set('display_errors',1);
error_reporting(E_ALL);
///////////
$file_csv = __DIR__.'/../members/test/1413369323.csv';
$file_pdf = __DIR__.'/../members/applications/1410381989.pdf';
$file_img = __DIR__.'/../members/images/1410381989.jpg';
$file_rcp = __DIR__.'/../members/payments/1410381989.jpg';

$files = array();
//тут заносим файлы в массив например так:
//$files[0]= "/directory/filename.jpg"
//$files[1]= "/directory/css/my.css"

$files[0]= $file_csv;
$files[1]= $file_pdf;
$files[2]= $file_img;
$files[3]= $file_rcp;

multi_attach_mail("yulik@ifn-solutions.com;4z5sl@iarc.org", $files, "info@iarc.org"); //вызываем функцию



function multi_attach_mail($to, $files, $sendermail){
    // email fields: to, from, subject, and so on
    $from = "Files attach <".$sendermail.">"; 
    $subject = "טפסי בקשה עבור: ".count($files); 
    $message = date("Y.m.d H:i:s")."\n".count($files)." צרופות\nדני, האם אתה רואה עברית בכל מקום ובתוך הקובץ?";
    $headers = "From: $from";

    // boundary 
    $semi_rand = md5(time()); 
    $mime_boundary = "==Multipart_Boundary_x{$semi_rand}x"; 

    // headers for attachment 
    $headers .= "\nMIME-Version: 1.0\n" . "Content-Type: multipart/mixed;\n" . " boundary=\"{$mime_boundary}\""; 

    // multipart boundary 
    $message = "--{$mime_boundary}\n" . "Content-Type: text/; charset=\"UTF-8\"\n" .
    "Content-Transfer-Encoding: 7bit\n\n" . $message . "\n\n"; 

    // preparing attachments
    for($i=0;$i<count($files);$i++){
        if(is_file($files[$i])){
            $message .= "--{$mime_boundary}\n";
            $fp =    @fopen($files[$i],"rb");
        $data =    @fread($fp,filesize($files[$i]));
                    @fclose($fp);
            $data = chunk_split(base64_encode($data));
            $message .= "Content-Type: application/octet-stream; name=\"".basename($files[$i])."\"\n" . 
            "Content-Description: ".basename($files[$i])."\n" .
            "Content-Disposition: attachment;\n" . " filename=\"".basename($files[$i])."\"; size=".filesize($files[$i]).";\n" . 
            "Content-Transfer-Encoding: base64\n\n" . $data . "\n\n";
            }
        }
    $message .= "--{$mime_boundary}--";
    $returnpath = "-f" . $sendermail;
    $ok = @mail($to, $subject, $message, $headers, $returnpath); 
    if($ok){ return $i; } else { return 0; }
    }
	
	
	
////////////
?>
	</font>
    </body>
</html>