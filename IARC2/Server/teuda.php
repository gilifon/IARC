...
if (isset ( $_POST ['submit'] )) {
	// Get data from POST and main routine
	...
	// Check if now December of current year
	$yearCurrent = date ( "Y" );
	$date = $yearCurrent . '-12';
	$now = time ();
	//if ($year == $yearCurrent) {
	...
	//		echo '<strong>The certificates for ' . $yearCurrent . ' year will be available in December.</strong>';
	//		die ();
	//	}
	//}
	
	$sql = "select * from hlwtest where `call` = '$callsign' and `year`='$year' and `active`=1";
	...
	$sql = "insert into hlcert (`call`, `year`, `ip`, `date`) VALUES ('$callsign', '$year', '$ip', now())";
