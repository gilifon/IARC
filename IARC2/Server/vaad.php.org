...
$result = mysqli_query($Link,"select `call`, name_heb, family_heb, cel, email from members WHERE `call` IN ($vaad_in)");
...
$result = mysqli_query($Link,"select `call`, name_heb, family_heb, cel, email from members WHERE `call` IN ($vaad_bikoret_in)");
while($obj = mysqli_fetch_object($result)) {
...
$result = mysqli_query($Link,"select `call`, name_heb, family_heb, cel, email from members WHERE `call` IN ($vaad_haverim_in)");
while($obj = mysqli_fetch_object($result)) {
