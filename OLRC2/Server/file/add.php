...
if (include '../isauthorized.php')
{
    ...
    $temp = mysqli_query($link, "SELECT MAX(`order`) as mo FROM `files` ");
    if (!$temp) {
    ...
    $sql="INSERT INTO `files` (`url`, `caption`, `order`) VALUES ('$newfile[t]','$newfile[d]',$result[mo]+1)";
    ...
    $body = 'A file has been added to the online course website (NOT attached here):<br/>'.$newfile['d'].'<br/> Please visite the online course<br/> http://iarc.org/lessons/#home';
    mail($to,'A file has been added',$body,$headers);
    ###########################################################################################################
    
    $sql="INSERT INTO `audit` (`username`, `tab`, `operation`) VALUES ('$name','file','add')";
