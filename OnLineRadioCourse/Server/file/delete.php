...
if (include '../isauthorized.php')
{
    ...
    $sql="UPDATE `files` SET `order`=`order`-1 where `id` > $file[id]";
    if (!mysqli_query($link, $sql))
    {
    ...
    $sql="INSERT INTO `audit` (`username`, `tab`, `operation`) VALUES ('$name','file','delete')";
    mysqli_query($link, $sql);
    mysqli_close($link);
    echo json_encode('The file has been deleted');
